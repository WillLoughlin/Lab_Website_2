...
function viewStudentStats(id, toggle){
	if (toggle == 0){//hide html tag
		document.getElementById(id).style.height = 0;
		document.getElementById(id).style.visibility = "hidden";
	}
	if (toggle == 1){//show html tag
...
function loadStatsPage(){
		...
		var winsT = document.getElementById("wins");
		var lossT = document.getElementById("losses");
		...
		{//Outer for loop iterates over each row
			for(col_counter = 1; col_counter < 4; col_counter++)
			{
				//alert(col_counter);
				if (col_counter == 1){
				...
				if (col_counter == 2){
					homeScore = parseInt(table.rows[row_counter].cells[col_counter].innerHTML);//Read in a cells current value
					//alert(table.rows[row_counter].cells[col_counter].innerHTML);
			  }
				if (col_counter == 3){
					oppScore = parseInt(table.rows[row_counter].cells[col_counter].innerHTML);//Read in a cells current value
					//alert(table.rows[row_counter].cells[col_counter].innerHTML);
				}
			...
			if (homeScore == oppScore){
				table.rows[row_counter].cells[col_counter].innerHTML = "Tie";
			}

		}
		winsT.innerHTML = wins;
		lossT.innerHTML = losses;
...
function switchPlayers(playerNum){
	...
	avgPassing.innerHTML = Math.floor(players[playerNum].pass_yards / players[playerNum].games_played);
	avgRushing.innerHTML = Math.floor(players[playerNum].rushing_yards / players[playerNum].games_played);
	avgReceiving.innerHTML = Math.floor(players[playerNum].receiving_yards / players[playerNum].games_played);
}
...
function loadPlayersPage(){
	...
	{
			playerToInsert = players[i].name;
			anchor = "<a class = 'dropdown-item' href='#' onclick = 'switchPlayers("+ i +")'>" + playerToInsert +"</a>";
			dropDown = dropDown + anchor;
	}

	playermenu.innerHTML = dropDown;
